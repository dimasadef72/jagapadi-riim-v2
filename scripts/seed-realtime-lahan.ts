...
if (
  ...
  throw new Error(
    "Konfigurasi Firebase Realtime Database belum lengkap di .env.",
  );
}
...
const lahanSeedData = [
  {
    fieldCode: "SW001",
    layers: { rgb: "Sawah1_RGB.jpg", ndvi: "Sawah1_NDVI.png" },
    corners: {
  ...
  {
    fieldCode: "SW002",
    layers: { rgb: "Sawah2_RGB.jpg", ndvi: "Sawah2_NDVI.png" },
    corners: {
    ...
    capturedAt: "2026-05-13T03:00:00.000Z",
    hama: {
      gridIndex: 266,
  ...
  {
    fieldCode: "SW003",
    layers: { rgb: "Sawah3_RGB.jpg", ndvi: "Sawah3_NDVI.png" },
    corners: {
    ...
    capturedAt: "2026-05-14T03:00:00.000Z",
    hama: {
      gridIndex: 388,
  ...
  {
    fieldCode: "SW004",
    layers: { rgb: "Sawah4_RGB.jpg", ndvi: "Sawah4_NDVI.png" },
    corners: {
...
function makeNdviMean(lahanIndex: number, rowIndex: number, colIndex: number) {
  ...
  const noise =
    (deterministicNoise((lahanIndex + 1) * 10000 + rowIndex * 97 + colIndex) -
      0.5) *
    0.16;
...
function makeGridBounds(
  ...
  const rowTopLeft = interpolatePoint(
    field.topLeft,
    field.bottomLeft,
    rowStartRatio,
  );
  const rowTopRight = interpolatePoint(
    field.topRight,
    field.bottomRight,
    rowStartRatio,
  );
  const rowBottomLeft = interpolatePoint(
    field.topLeft,
    field.bottomLeft,
    rowEndRatio,
  );
  const rowBottomRight = interpolatePoint(
    field.topRight,
    field.bottomRight,
    rowEndRatio,
  );
...
function createRealtimeLahanTree(
  field: (typeof lahanSeedData)[number],
  lahanIndex: number,
) {
  const grids: Record<string, unknown> = {};
  const inspectionPoints: Record<string, unknown> = {};
  const hamaDetections: Record<string, unknown> = {};
  const gridRows = [];
  ...
  for (let rowIndex = 0; rowIndex < ROW_COUNT; rowIndex += 1) {
    for (let colIndex = 0; colIndex < COL_COUNT; colIndex += 1) {
      ...
      gridRows.push({ gridCode: code, bounds, ndviMean, stress });
    }
  }
  ...
  for (let pointIndex = 0; pointIndex < 4; pointIndex += 1) {
    ...
    for (const grid of pointGrids) {
      const gridNumber = Number(grid.gridCode.match(/\d+$/)?.[0] ?? 1);
      const readingId = `reading_${grid.gridCode}`;

      sensorReadings[readingId] = {
        pointCode,
        latitude: grid.bounds.centerLat,
        longitude: grid.bounds.centerLng,
        co2Ppm: round(
          405 + grid.stress * 190 + deterministicNoise(gridNumber + 11) * 35,
          1,
        ),
        nh3Ppm: round(
          0.08 + grid.stress * 0.7 + deterministicNoise(gridNumber + 23) * 0.08,
          3,
        ),
        coPpm: round(
          0.35 + grid.stress * 1.7 + deterministicNoise(gridNumber + 37) * 0.2,
          3,
        ),
        no2Ppm: round(
          0.02 +
            grid.stress * 0.18 +
            deterministicNoise(gridNumber + 41) * 0.025,
          3,
        ),
        temperatureC: round(
          27.2 + grid.stress * 6.2 + deterministicNoise(gridNumber + 53) * 1.1,
          1,
        ),
        humidityPct: round(
          84 - grid.stress * 22 - deterministicNoise(gridNumber + 67) * 4,
          1,
        ),
        recordedAt: new Date(
          Date.UTC(2026, 4, 15 + lahanIndex, 2, gridNumber % 60),
        ).toISOString(),
      };
    }

    inspectionPoints[pointCode] = {
      pointCode,
      clusterId,
  ...
  hamaDetections.detection_001 = {
    ...
    detectedAt: new Date(
      Date.UTC(2026, 4, 18 + lahanIndex, 3, 0),
    ).toISOString(),
  };
  ...
  return {
    ...
    hama_detections: hamaDetections,
  };
}

async function seedRealtimeLahan() {
  let totalGrids = 0;
  ...
  for (const [lahanIndex, field] of lahanSeedData.entries()) {
    await remove(ref(db, `lahan/${field.fieldCode}`));

    const data = createRealtimeLahanTree(field, lahanIndex);
    await set(ref(db, `lahan/${field.fieldCode}`), data);

    totalGrids += Object.keys(data.grids).length;
    totalInspectionPoints += Object.keys(data.inspection_points).length;
    totalSensorReadings += Object.values(data.inspection_points).reduce<number>(
      (sum, point) => {
        const sensorReadings = (
          point as { sensor_readings: Record<string, unknown> }
        ).sensor_readings;

        return sum + Object.keys(sensorReadings).length;
      },
      0,
    );
    totalHama += Object.keys(data.hama_detections).length;
  }

  console.log(
    [
      "Seed Realtime Database lahan selesai.",
      `Lahan: ${lahanSeedData.length}`,
      `Grid: ${totalGrids}`,
      `Inspection point: ${totalInspectionPoints}`,
      `Sensor reading: ${totalSensorReadings}`,
